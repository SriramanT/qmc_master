...
Band theory made it made possible to predict whether a solid would be a metal or an insulator. However, its success of band theory rests crucially on the independent electron approximation. Thus, it is not surprising that for compounds with strongly correlated electrons the theory might fail. 
...
\begin{equation}
Z_{\bm h} = (c_U)^{NL} \text{Tr}_{\bm h} \text{det} [ \bm M_\uparrow (\bm h) \bm M_\downarrow (\bm h) ] ,
\end{equation}
...
\begin{equation}
P(\bm h) = \frac{A}{Z_h} \det [ M_{\uparrow}(\bm h) ] \det [ M_{\downarrow}(\bm h) ] ,
\end{equation}
